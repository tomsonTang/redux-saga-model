...
var SagaModel = exports.SagaModel = function () {
  ...
  (0, _createClass3.default)(SagaModel, [{
    ...
    value: function createEffects(model) {
      /**
       * 对 saga 的 put 进行封装，默认分发的 action 被当前 namespace 捕获,
       * 若需要对其他 namespace 进行分发，需要指定 namespace
       *
       * @param {any} action
      ...
      function put(action, namespace) {
        ...
        return sagaEffects.put((0, _extends3.default)({}, action, {
          type: namespace ? "" + namespace + SEP + type : this.prefixType(type, model)
        }));
      }

      /**
      * 对 saga 的 take 进行封装，默认捕获的 action 为当前 namespace ,
       * 若需要对其他 namespace 进行捕获，需要指定 namespace
       *
       * @param {any} pattern
       * @param {any} namespace 其他 namespace
       * @returns
